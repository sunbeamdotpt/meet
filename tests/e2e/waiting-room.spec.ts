...
test('host can admit a waiting guest', async ({ browser }) => {
  ...
  await expect(guestPage.locator('[data-testid="waiting-screen"]')).not.toBeVisible({ timeout: 20000 });
  await expect(guestPage.locator('[data-testid="meeting-controls"]')).toBeVisible({ timeout: 20000 });
