...
test('host can send a reaction that appears in the overlay', async ({ page }) => {
  ...
  await signInWithTestAccount(page, { name: 'Reaction Host', email: 'reaction-host@example.com', roomName, role: 'host' });
  await joinRoom(page);
  ...
  await expect(page.locator('[data-testid="reaction-overlay"] span')).toContainText('👍', { timeout: 5000 });
});
